...
def compute_pid(Goal, Current_value, pid_state):
    ...
    # Compute angle of required tilt direction
    theta = math.degrees(math.atan2(output_y, output_x))

    # === Camera-to-Arm Alignment Adjustment ===
    # To map the visual frame to the mechanical coordinate system:
    # - Temporarily rotate the frame using the 'angle' variable (currently set to 0).
    # - Adjust this 'angle' until Arm1 aligns perfectly with the visual X-axis.
    # - Once aligned (e.g., angle=30), set angle back to 0, and **add that alignment angle** (e.g., +30) here.
    # - This ensures the direction of tilt matches the servo layout.
    theta += 30  # Replace 30 with the alignment angle determined visually
    if theta < 0:
        theta += 360
